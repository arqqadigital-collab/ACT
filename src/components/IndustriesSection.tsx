      ...
      <div className="container-width relative z-10">
        ...
        >
          ...
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">
            Industries We Empower with <span className="text-gradient">Technology</span>
          </h2>
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            ACT Combines Global Partnerships With Local Expertise to Deliver Tailored ICT and Digital Transformation
            Solutions Across Egypt, the Middle East, and Beyond.
          </p>
        </div>
        ...
        >
              ...
              {industries.map((industry, index) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    <h3 className="font-display text-base font-bold text-white mb-1">{industry.title}</h3>
                    <p className="text-white/80 text-xs line-clamp-2">{industry.description}</p>
                  </div>
                </Link>
              ))}
            </div>
          ...
          <div className="hidden sm:grid sm:grid-cols-3 gap-3 md:hidden">
            {industries.map((industry, index) => (
              <Link key={index} to={industry.link} className="relative overflow-hidden rounded-2xl aspect-[3/4]">
                {/* Background Image */}
                <div className="absolute inset-0">
                  <img src={industry.image} alt={industry.title} className="w-full h-full object-cover" />
                </div>
            ...
            {industries.map((industry, index) => (
              <Link
                ...
                className={`relative overflow-hidden cursor-pointer transition-all duration-500 ease-out ${
                  activeIndex === index ? "flex-[3]" : activeIndex !== null ? "flex-[0.5]" : "flex-1"
                }`}
              ...
              >
                ...
                <div
                  className={`absolute bottom-0 left-0 right-0 p-6 transition-all duration-500 ${
                    activeIndex === index ? "translate-y-0 opacity-100" : "translate-y-4"
                  }`}
                >
                  ...
                  <p
                    className={`text-white/80 text-sm transition-all duration-500 ${
                      activeIndex === index ? "opacity-100 max-h-20" : "opacity-0 max-h-0"
                    }`}
                  >
