    ...
    >
      ...
      <div
        ...
        style={{
          ...
          transition:
            "height 0.7s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.3s ease",
        }}
      />
      ...
      >
        <div
          className={`px-6 py-2.5 rounded-full font-bold text-base md:text-lg transition-all duration-300 shadow-lg ${
            isOpen
      ...
      <div
        className={`overflow-hidden transition-all duration-700 ease-out ${
          isOpen ? "max-h-[2000px] opacity-100" : "max-h-0 opacity-1"
        }`}
        style={{
          transition:
            "max-height 0.7s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.5s ease-out",
        }}
      >
          ...
          {era.events.map((event, eventIndex) => (
            <div
              ...
              className={`relative transition-all duration-500 ${
                isOpen ? "opacity-100 translate-y-0" : "opacity-1 translate-y-4"
              }`}
              style={{
                transitionDelay: isOpen ? `${eventIndex * 120}ms` : "0ms",
              }}
            >
              {/* Event Card */}
