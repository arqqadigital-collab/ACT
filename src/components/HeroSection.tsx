...
const HeroSection = () => {
  ...
  const headline = "Empowering Organizations with Trusted Technology Solutions in Egypt & Beyond";
  ...
  const renderHeadline = () => {
    ...
    if (startIndex === -1) {
      return (
        <>
          {displayedText}
          {!isComplete && <span className="inline-block w-0.5 h-[1em] bg-primary animate-pulse ml-1" />}
        </>
      );
    }
    
    const before = displayedText.slice(0, startIndex);
    const gradientText = displayedText.slice(startIndex, startIndex + gradientPhrase.length);
  ...
  return (
    <section className="relative h-screen flex flex-col items-center justify-center overflow-hidden bg-background">
      ...
      <div className="absolute bottom-1/4 right-1/4 w-40 sm:w-56 md:w-80 h-40 sm:h-56 md:h-80 rounded-full bg-accent/10 blur-[80px] animate-glow-pulse z-[1]" style={{ animationDelay: '1.5s' }} />
      ...
      >
        ...
        <p className={`text-lg md:text-xl text-muted-foreground max-w-3xl mx-auto mb-10 leading-relaxed transition-all duration-700 ${
